$(document).ready(function() {
	...
	$(window)
		...
		.keyup(function(event) {
			...
			// CHECK WHAT'S GOT FOCUS (TEXTAREA ISSUES)
	...
	MathJax.Hub.queue.Push(function() {
		...
		$('.MathJax').parents('[contenteditable="true"]')
			// Edit content with maths
			.focus(function() {
			
				// For each instance of MathJax
				$.each(MathJax.Hub.getAllJax($(this).get(0)), function(key, elementJax) {
				
					// Show the original TeX
					$(elementJax.SourceElement()).after(elementJax.originalText);
					
					// Remove formatted math
					elementJax.Remove();
				
				});
				
				// Remove script tags
				$(this).find('script[type^="math/tex"]').remove();
			
			})
			// Finished editing content that had maths
			.blur(function() {
